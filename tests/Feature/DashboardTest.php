...
test('dashboard page layout matches specifications', function () {
    ...
    expect($dashboardPage)
        ...
        ->toContain('primaryLink')
        ->toContain('grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4')
...
test('dashboard includes summary metrics and view chart for links owned by the user', function () {
    ...
    $this->actingAs($owner)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('linksCount', 1)
            ->where('primaryLink.display_name', 'Owned Profile')
            ->where('primaryLink.slug', $ownedLink->slug)
