...
const props = defineProps<{
    ...
    widgetStyle: 'sharp' | 'soft' | 'rounded';
...
const emit = defineEmits<{
    ...
    'update:pageTheme': [theme: 'light' | 'dark'];
    'update:widgetStyle': [style: 'sharp' | 'soft' | 'rounded'];
    addLink: [];
    addMap: [];
    addMedia: [file: File];
    addSection: [];
...
const closeStylePanelOnOutsideClick = (event: PointerEvent) => {
    ...
    if (
        ...
        (stylePanelRef.value?.contains(target) ||
            targetElement?.closest('[data-style-toggle-button]'))
    ) {
        return;
    }
...
<template>
    ...
    >
        <div
            ref="stylePanelRef"
            v-if="isEditing && showStylePanel && !mobileWidgetOperationActive"
                ...
                <div class="grid gap-3 border-b border-gray-200 pb-4">
                    ...
                    >
                        <button
                            ...
                            "
                            @click="emit('update:pageTheme', option.value)"
                        >
                ...
                <div class="grid gap-3">
                    ...
                    >
                        <button
                            ...
                            @click="emit('update:widgetStyle', option.value)"
                        >
                            <span
        ...
        >
            <Button
                v-if="!isEditing && !isPublished && hasWidgets"
                variant="ghost"
                class="h-8 cursor-pointer gap-2 rounded-xl bg-black px-4 text-sm text-white hover:bg-neutral-800 hover:text-white"
                @click="emit('publish')"
            >
            ...
            <Button
                ...
                class="h-8 cursor-pointer gap-2 rounded-xl px-4"
                :class="
                    isEditing
            ...
            <template v-if="isEditing">
                ...
                >
                    ...
                    <span class="font-semibold">マップ</span>
                </Button>
                <Button
                    variant="ghost"
                    class="h-8 cursor-pointer gap-2 rounded-xl px-4 text-sm text-gray-900"
        ...
        >
            ...
            <button
                @click="emit('toggleEdit')"
                class="flex h-8 cursor-pointer items-center justify-center rounded-xl transition-colors"
                :class="
                    isEditing
            ...
            <template v-if="isEditing">
                ...
                <div class="hs-tooltip inline-block">
                    <button
                        ...
                        aria-label="スタイル変更"
                        title="スタイル変更"
                        class="hs-tooltip-toggle flex size-8 cursor-pointer items-center justify-center rounded-xl transition-colors"
                        :class="
                            showStylePanel
                                ? 'bg-[#292929] text-white shadow-sm'
                        ...
                        @click.stop="showStylePanel = !showStylePanel"
                    >
                        <Palette class="size-5" stroke-width="2.2" />
                    </button>
                    <span
                        class="hs-tooltip-content invisible absolute z-10 inline-block rounded-md bg-gray-900 px-2 py-1 text-xs whitespace-nowrap text-white opacity-0 transition-opacity hs-tooltip-shown:visible hs-tooltip-shown:opacity-100"
                        role="tooltip"
                    >
                        スタイル変更
                    </span>
                ...
                <div class="hs-tooltip inline-block">
                    <button
                        aria-label="リンクを追加"
                        @click="emit('addLink')"
                        class="hs-tooltip-toggle ml-2 flex size-8 cursor-pointer items-center justify-center rounded-xl transition-colors hover:bg-slate-100 hover:text-slate-800"
                    >
                ...
                <div class="hs-tooltip inline-block">
                    <button
                        ...
                        class="hs-tooltip-toggle ml-2 flex size-8 cursor-pointer items-center justify-center rounded-xl transition-colors hover:bg-slate-100 hover:text-slate-800"
                    >
                        <MapPinned class="size-5" stroke-width="2.2" />
                    </button>
                    <span
                        class="hs-tooltip-content invisible absolute z-10 inline-block rounded-md bg-gray-900 px-2 py-1 text-xs whitespace-nowrap text-white opacity-0 transition-opacity hs-tooltip-shown:visible hs-tooltip-shown:opacity-100"
                        role="tooltip"
        ...
        >
            ...
            >
                <button
                    ...
                    class="hs-tooltip-toggle flex size-8 cursor-pointer items-center justify-center rounded-lg transition-colors"
                    :class="
                ...
                >
                    <span
                        v-if="
                            activeMobileWidget?.type !== 'section' &&
                            option.key !== 'inline'
                        "
